...
int main(int argc, char* argv[])
{
  ...
  memset(&sigact_usr, '\0', sizeof(sigaction));

  sigact.sa_handler = timer_handler;
  sigact_usr.sa_handler = timer_handler_usr;
  
  if(-1 == sigaction(SIGALRM, &sigact, NULL))
    perror("Could not install signal handler");
  
  if(-1 == sigaction(SIGUSR1, &sigact_usr, NULL))
    perror("Could not install signal_usr handler");
