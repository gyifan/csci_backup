...
#ifdef DEBUG
	printf("==========================================\n");
	printf("temp is set: %p\n", temp);
	printf("tData: %p\n", tData);
#endif /* DEBUG */

	while((temp!=NULL) && (tData->size < size))
		temp=temp->next;
	tData=(struct block *)temp->data;
	if(temp==NULL){
...
int deallocate(void *ptr){
	...
	/* Tries to find consecutive block in free_list */
	struct dnode *counter;
	struct block *cData;
	...
	while(counter!=NULL){
		...
		/* check if free block before newblk */
		if(cData->ptr+cData->size == newblk->ptr){
			front = counter;
			fData = cData;
		}
		/* check if free block after newblk */
		if(newblk->ptr+newblk->size == cData->ptr){
			back = counter;
			bData = cData;
		}
	...
	if(front==NULL){
		if(back==NULL)
			/* No consecutive pointer in free_list */
			dlist_add_front(free_list, newblk);
		else { /* ONLY consecutive block after newblk */
			temp=back;
			tData = (struct block *)temp->data;
			tData->ptr = newblk->ptr;
			tData->size += newblk->size;
			dnode_setdata(temp,tData);
			free(newblk);
		}
	}else {
		if(back==NULL){ /*ONLY consecutive block before newblk */
			temp=front;
			tData = (struct block *)temp->data;
			tData->size += newblk->size;
			dnode_setdata(temp,tData);
			free(newblk);
		} else { /* Two consecutive block before and after */
			temp=front;
			tData = (struct block *)temp->data;
			tData->size += (newblk->size+bData->size);
			dnode_setdata(temp,tData);
			temp->next=back->next;
			dnode_destroy(back);
			free(newblk);
